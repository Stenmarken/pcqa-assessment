...
def plot_scores(path):
    ...
    plt.scatter([], [], c="red", label="Heavy fog")
    plt.scatter([], [], c="yellow", label="Moderate fog")
    plt.scatter([], [], c="green", label="Light fog")
    plt.legend()

    plt.xlabel("Point cloud index")
    plt.ylabel("Score")
    plt.title(
        "Plot showing scores of point clouds 1.ply - 100.ply at three different severities"
    )
    plt.grid(True)
